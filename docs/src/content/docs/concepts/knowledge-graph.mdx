...
Lucerna extracts a directed relationship graph from the AST alongside chunks. Edges are stored in a LanceDB table and can be traversed with the [graph traversal API](/programmatic-usage/graph-traversal) or the [`lucerna graph`](/cli/graph) CLI command.
