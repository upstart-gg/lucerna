...
Put your provider in any `.ts` or `.js` file and import it directly from `lucerna.config.ts`. No build step or publishing required — Lucerna's config loader handles TypeScript transpilation automatically.
...
export default defineConfig({
  embedding: new MyEmbeddings({ apiKey: process.env.MY_API_KEY }),
  reranking: new MyReranker(),
});
```
