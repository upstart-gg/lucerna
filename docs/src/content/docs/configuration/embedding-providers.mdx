...
export default defineConfig({
  embedding: {
    ...
    dimensions: 256, // truncate from native 3072 for a ~12× smaller index
  },
});
```
...
| Model | Dimensions | Price per 1M tokens | Notes |
|-------|-----------|---------------------|-------|
| `gemini-embedding-2-preview` | 3072 (truncatable 128–3072) | — | 8K ctx, multi-modal, latest |
| `gemini-embedding-001` | 3072 (truncatable 128–3072) | $0.15 (batch $0.075) | 2K ctx, text only |
| `text-embedding-004` | 768 | $0.15 | Legacy, 2K ctx |
...
Both `gemini-embedding-*` models support Matryoshka truncation via `dimensions` (128–3072). When `dimensions` is below the native 3072, Lucerna L2-normalizes each returned vector as required by the API.
...
| Model | Dimensions | Price per 1M tokens | Notes |
|-------|-----------|---------------------|-------|
| `gemini-embedding-001` | 3072 (truncatable 128–3072) | — | Highest quality, 1 input per request |
| `text-embedding-005` | 768 | $0.10 | Latest text model, up to 250 inputs per request |
