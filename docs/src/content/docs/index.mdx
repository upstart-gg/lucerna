...
:::tip[Why Lucerna?]
Most AI coding agents are flying blind. They read files one at a time, guess at structure, and miss the connections that make a codebase make sense. Lucerna gives agents a map: instant semantic search, exact symbol lookup, and a knowledge graph that knows what calls what, what imports what, and what everything relates to — so agents spend less time exploring and more time actually helping.

Lucerna works with any MCP-compatible agent like Claude Code, OpenCode, Cursor, Windsurf, and more.
:::
