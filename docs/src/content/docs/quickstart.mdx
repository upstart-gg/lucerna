...
## Configuring embeddings and reranking

Place a `lucerna.config.ts` at the project root to configure providers — the MCP server picks it up automatically:
...
Or pass flags directly in the MCP server command:
...
See [Configuration](/configuration/overview) and [Custom Providers](/configuration/custom-providers) for full details.
