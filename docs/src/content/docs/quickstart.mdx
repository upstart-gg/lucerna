...
The MCP server requires **no configuration**. When it starts, it indexes the project and begins watching for file changes automatically — no separate `lucerna watch` process needed, no API keys required.
:::
...
To improve search quality with custom embedding or reranking providers, see [Configuration](/configuration/overview).
