...
const CPP_INHERIT_QUERIES = {
  // base_class_clause lists parent classes — one match per base (multiple inheritance)
  extends: `(class_specifier name: (type_identifier) @name (base_class_clause (type_identifier) @base)) @cls`,
};
...
export function extractCpp(
  ...
  try {
    extracted = packExtract(source, {
      ...
      patterns: Object.fromEntries(
        Object.entries(CPP_QUERIES).map(([k, q]) => [
          k,
  ...
  try {
    ...
    const inh: any = packExtract(source, {
      language: "cpp",
      patterns: Object.fromEntries(
        Object.entries(CPP_INHERIT_QUERIES).map(([k, q]) => [
          k,
          { query: q, captureOutput: "Full" },
        ]),
      ),
