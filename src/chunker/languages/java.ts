...
const JAVA_INHERIT_QUERIES = {
  extends: `(class_declaration name: (identifier) @name (superclass (type_identifier) @base)) @cls`,
  implements: `(class_declaration name: (identifier) @name (super_interfaces (type_list (type_identifier) @iface))) @cls`,
};
...
export function extractJava(
  ...
  try {
    extracted = packExtract(source, {
      ...
      patterns: Object.fromEntries(
        Object.entries(JAVA_QUERIES).map(([k, q]) => [
          k,
  ...
  try {
    ...
    const inh: any = packExtract(source, {
      language: "java",
      patterns: Object.fromEntries(
        Object.entries(JAVA_INHERIT_QUERIES).map(([k, q]) => [
          k,
          { query: q, captureOutput: "Full" },
        ]),
      ),
