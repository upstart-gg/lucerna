...
const SOLIDITY_QUERIES = {
  imports: `(import_directive source: (string_literal) @module) @imp`,
  ...
  callExpressions: `(call_expression (identifier) @callee) @call`,
};
...
export function extractSolidity(
  ...
  const addChunk = (
    ...
    name: string,
    type: "class" | "interface" | "function" | "method" | "type",
    parentName?: string,
  ) => {
  ...
  // events and modifiers map to type
  for (const key of ["events", "modifiers"] as const) {
    const capture = key === "events" ? "event" : "modifier";
    for (const m of getMatches(key)) {
      const node = cap(m, capture)?.node;
      const name = cap(m, "name")?.text ?? "";
