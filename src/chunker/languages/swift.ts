...
const SWIFT_INHERIT_QUERIES = {
  ...
  inherits: `(class_declaration name: (type_identifier) @name (inheritance_specifier) @base) @cls`,
};

// Note: in this grammar version, class_declaration represents struct, class,
// and enum. Protocols use protocol_declaration. Enums are distinguished by the
// "enum" keyword in the query and are processed before classes to deduplicate.
export function extractSwift(
  ...
  try {
    extracted = packExtract(source, {
      ...
      patterns: Object.fromEntries(
        Object.entries(SWIFT_QUERIES).map(([k, q]) => [
          k,
  ...
  try {
    ...
    const inh: any = packExtract(source, {
      language: "swift",
      patterns: Object.fromEntries(
        Object.entries(SWIFT_INHERIT_QUERIES).map(([k, q]) => [
          k,
          { query: q, captureOutput: "Full" },
        ]),
      ),
    ...
    for (const m of inhMatches("inherits")) {
