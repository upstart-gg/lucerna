...
program
  ...
  .action(async (opts: Record<string, unknown>) => {
    const resolvedRoot = resolve(String(opts.dir ?? process.cwd()));
    let filesIndexed = 0;
...
program
  ...
  .action(async (query: string, opts: Record<string, unknown>) => {
    const resolvedRoot = resolve(String(opts.dir ?? process.cwd()));
    const indexer = await buildIndexer(resolvedRoot, opts);
    try {
      await indexer.initialize();
...
program
  ...
  .action(async (chunkId: string, opts: Record<string, unknown>) => {
    const resolvedRoot = resolve(String(opts.dir ?? process.cwd()));
    const indexer = await buildIndexer(resolvedRoot, {
      ...opts,
      semantic: false,
    });
    try {
      ...
      else if (relation === "usages") chunks = await indexer.getUsages(chunkId);
      else {
        console.error(
...
program
  ...
  .option("--format <fmt>", "Output format: raw, json, or pretty-json", "raw")
  .option("--no-semantic", "Disable semantic (vector) search — lexical only")
  .action(async (queriesFile: string, opts: Record<string, unknown>) => {
...
program
  ...
  .action(async (opts: Record<string, unknown>) => {
    ...
    await startMcpServer(resolve(String(opts.dir ?? process.cwd())), {
      ...(opts.storageDir
...
async function buildIndexer(
  ...
  // include/exclude: CLI flag > config file
  const resolvedInclude = opts.include
    ? String(opts.include)
        .split(",")
        .map((s) => s.trim())
    : cfg.include;
  const resolvedExclude = opts.exclude
    ? String(opts.exclude)
        .split(",")
        .map((s) => s.trim())
    : cfg.exclude;
