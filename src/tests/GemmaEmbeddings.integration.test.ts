...
//
// NOTE: The underlying model (onnx-community/embeddinggemma-300m-ONNX) is
// subject to the Google Gemma Terms of Use: https://ai.google.dev/gemma/terms
//
// Run with: INTEGRATION_TESTS=1 bun test src/tests/GemmaEmbeddings.integration.test.ts
// ---------------------------------------------------------------------------

const SKIP_INTEGRATION = process.env.INTEGRATION_TESTS !== "1";

describe.skipIf(SKIP_INTEGRATION)("GemmaEmbeddings — integration", () => {
  const embeddings = new GemmaEmbeddings();
  ...
  test("warmup() pre-loads so generate() reuses the pipeline", async () => {
    const fresh = new GemmaEmbeddings();
    await fresh.warmup();
