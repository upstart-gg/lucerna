...
//
// Run with: INTEGRATION_TESTS=1 bun test src/tests/JinaCodeEmbeddings.integration.test.ts
// ---------------------------------------------------------------------------

const SKIP_INTEGRATION = process.env.INTEGRATION_TESTS !== "1";

describe.skipIf(SKIP_INTEGRATION)("JinaCodeEmbeddings — integration", () => {
  const embeddings = new JinaCodeEmbeddings();
  ...
  test("warmup() pre-loads so generate() reuses the pipeline", async () => {
    const fresh = new JinaCodeEmbeddings();
    await fresh.warmup();
