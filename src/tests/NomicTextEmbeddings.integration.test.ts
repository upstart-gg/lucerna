...
//
// Run with: INTEGRATION_TESTS=1 bun test src/tests/NomicTextEmbeddings.integration.test.ts
// ---------------------------------------------------------------------------

const SKIP_INTEGRATION = process.env.INTEGRATION_TESTS !== "1";

describe.skipIf(SKIP_INTEGRATION)("NomicTextEmbeddings — integration", () => {
  const embeddings = new NomicTextEmbeddings();
  ...
  test("warmup() pre-loads so generate() reuses the pipeline", async () => {
    const fresh = new NomicTextEmbeddings();
    await fresh.warmup();
